...
class Tile:
    ...
    def set_possible_positions(self, n, m):
        ...
        self.possible_positions_only_flip_and_rotation = [
            entry for entry in self.possible_positions if array_to_tuple(entry) not in set([array_to_tuple(entry2) for entry2 in self.possible_positions_no_flip_and_rotation])
        ]
        print(
            f"Removed {number_of_solutions-len(self.possible_positions)} duplicate positions. Found {len(self.possible_positions)} unique possible positions! \n\
    ...
    def generate_position_dict(self, matrix_list):
        ...
        for i, matrix in enumerate(matrix_list):
            ...
            positions_dict[i] = {"position": matrix, "diagonal_coordinates": diagonal_coordinates}
